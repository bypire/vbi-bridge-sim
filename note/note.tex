...
{\raggedright
  {\LARGE\bfseries Time-step restrictions for explicit integration of damped
   moving-load beam problems, and two remedies}\\[0.4em]
  {\large A short, fully reproducible note}\\[0.8em]
...
\noindent\colorbox{rwthblue25}{%
  ...
  the explicit step by a factor of \(\approx 13\), making speed sweeps and
  Monte-Carlo studies needlessly expensive. Two standard remedies remove the
...
on a damped finite-element (FE) beam the condition is far more restrictive than a
first reading suggests. This note isolates and quantifies that restriction on a
concrete bridge, and verifies the two standard cures. Nothing here is a new
method; the contribution is a clean, reproducible quantification with every
number tied to an analytical reference.
...
Stiffness-proportional damping makes this worse in a way that is easy to miss.
...
and \emph{reaches the same answer}: DAF \(1.0800\) versus \(1.0799\), a
...
of explicit integrators applied to stiffly damped systems; the value of this note
is the clean, reproducible quantification in the moving-load VBI setting and the
side-by-side verification of the two cures, not a new integrator. Mass-proportional
...
re-derived. The remedies are standard: Newmark-\(\beta\) is the textbook
structural integrator, and modal truncation is elementary model order reduction;
the point is simply that either removes a restriction that is easy to pay for
unknowingly.
